...
# Create and start the container with the name *sn-map-playwright*
...
docker run -d --name sn-map-playwright --rm --network host -v $(pwd):/work/ -w /work/ -it mcr.microsoft.com/playwright:v$playwrightVersion-focal

# The actual generate/update the reference screenshots is running from the machine running this shell script
docker exec sn-map-playwright /bin/sh -c "yarn test:rendering --update-snapshots"

docker stop sn-map-playwright
