...
describe('number-format-properties utils', () => {
  ...
  it('determine if the number format properties should be shown', () => {
    const numFormatProps2 = NumberFormatProperties.getNumberFormattingPropertiesWithMasterLink(
      'test.ref',
      true,
      isLink
    );
    expect(numFormatProps2.items.numberFormat.show(props)).toEqual(true);
    isLink = () => true;
    props.test = {
      ref: {
        quarantine: {
    ...
    const numFormatProps = NumberFormatProperties.getNumberFormattingPropertiesWithMasterLink('test.ref', true, isLink);
    expect(numFormatProps.items.numberFormat.show(props)).toEqual(false);
  });

  it('set the correct refs', () => {
    const ref = 'test.something.now';
    const numFormatProps = NumberFormatProperties.getNumberFormattingPropertiesWithMasterLink(
      'test.something.now',
      true,
      isLink
    );
