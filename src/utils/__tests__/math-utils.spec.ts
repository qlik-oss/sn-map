...
describe('math-utils function tests', () => {
  it('should calculate arc', () => {
    const p1: [number, number] = [0, 0];
    let p2: [number, number] = [1, 0];
    let curviness: number = 0;
    const isLatLong = false;

    expect(MathUtils.makeLineArc(p1, p2, curviness, isLatLong)).toEqual([
      [0, 0],
      [1, 0],
    ]);
    expect(MathUtils.makeLineArc(p1, p2, curviness, isLatLong).length).toEqual(2);
    ...
    curviness = 20;
    p2 = [10, 10];
    expect(MathUtils.makeLineArc(p1, p2, curviness, isLatLong).length).toBeGreaterThanOrEqual(3);
  });
